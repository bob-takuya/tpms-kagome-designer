...
import type { ImplicitFunction } from './tpms';

// Edge table for marching cubes - maps cube configuration to edges
const edgeTable = [
...
// Triangle table - maps cube configuration to triangles
const triTable = [
  [],
  [0, 8, 3],
  [0, 1, 9],
...
// Edge vertices - which cube vertices each edge connects
...
function interpolateVertex(
  ...
  if (Math.abs(v1) < 1e-10) return p1.clone();
  if (Math.abs(v2) < 1e-10) return p2.clone();
...
export function marchingCubes(
  ...
  const scale = period / (2 * Math.PI);
  const scaledMin = minBound * scale;
  const scaledMax = maxBound * scale;
  const step = (scaledMax - scaledMin) / resolution;
  ...
  for (let i = 0; i <= resolution; i++) {
    ...
    for (let j = 0; j <= resolution; j++) {
      ...
      for (let k = 0; k <= resolution; k++) {
        const x = scaledMin + i * step;
        const y = scaledMin + j * step;
        const z = scaledMin + k * step;
        // Use unscaled coordinates for the implicit function
        fieldValues[i][j][k] = implicitFunc(x / scale, y / scale, z / scale);
      }
    }
  }
    ...
    for (let j = 0; j < resolution; j++) {
      for (let k = 0; k < resolution; k++) {
        ...
        for (let c = 0; c < 8; c++) {
          ...
          cubeValues.push(fieldValues[i + di][j + dj][k + dk]);
        }

        // Calculate cube index
        let cubeIndex = 0;
        for (let c = 0; c < 8; c++) {
          if (cubeValues[c] < 0) cubeIndex |= (1 << c);
        }
        ...
        for (let t = 0; t < triangles.length; t += 3) {
          ...
          for (let v = 0; v < 3; v++) {
            const vertex = edgeVerts[triangles[t + v]]!;
  ...
  const normalCounts = new Float32Array(vertices.length / 3);
